...
1. 已部署提交号与远程 `origin/main` 一致。
2. 构建版本号与待发布代码一致。
3. 源码指纹与待发布文件一致。
4. 服务进程启动时间已更新。
5. 正式环境由 Gunicorn 提供服务，不是 Python WSGIServer 开发服务。
6. 连续多次公网请求均返回相同提交、版本与指纹，避免部分工作进程仍为旧版。

任何一项失败，本次发布都必须按失败处理，不得报告“已更新完成”。

正式发布包中的 `.release-commit` 是本次候选版本的权威提交号。即使正式运行目录残留旧版 `.git` 元数据，也必须以该发布标记为准；直接从 Git 工作区激活时，激活脚本会先将该标记刷新为当前 `HEAD`。这样可以避免新文件已经到达服务器，却因读取到旧 Git 信息而被误判并回滚。

发布包同时记录 `.release-generation` 单调版本序号。激活脚本会先读取当前运行服务的版本序号；如果延迟到达的 Webhook 携带更旧序号，必须在安装依赖和重启服务前拒绝该任务，防止并行发布乱序导致公网回退。

注意：交付包必须同时包含 `catalog_wsgi.py` 和 `deploy/`。缺少任何一项都会让服务器继续使用旧启动入口或旧 systemd 配置。

`rachel-catalog-post-release-verify.service` 和 `.timer` 是为兼容旧版服务器部署脚本保留的退役单元。它们不会执行30分钟复验，也不会由正式服务启动；不要将它们误认为延迟验收机制已经恢复。
...
```text
本机检查通过 -> origin/main -> 公司服务器自动部署 -> 公网验收通过 -> github/main 备份
```

1. 本机先通过本次功能测试、发布测试、Python 编译和 Git 格式检查，并且工作区必须干净。
2. `scripts/publish_verified_release.sh` 将锁定的提交推送到 `origin/main`，由既有 Webhook 自动通知公司服务器，日常发布不需要服务器管理员操作。
3. 公司服务器自动拉取 `origin/main`，刷新可追溯发布信息、强制重启 Gunicorn，并执行正式激活检查。
4. 本机发布脚本等待公网切换到同一提交，再连续抽查 256 次并覆盖 8 个 Gunicorn 工作进程。
5. 只有公网提交号、发布序号、构建号、源码指纹和运行方式全部通过，脚本才把该精确提交快进推送到 `github/main`。
6. 任一环节失败都会停止流程；GitHub 保留上一次通过公网验收的版本，不得备份未验收版本。

`github` 只保存最近一次通过公网验收的代码，不连接公司服务器，也不触发部署。发布使用：

```bash
./scripts/publish_verified_release.sh <本次修改对应的 unittest 名称...>
```

脚本始终推送启动发布时锁定的提交号。即使等待部署期间本机产生后续提交，也不会把未经公网验收的新代码误推到 GitHub。
