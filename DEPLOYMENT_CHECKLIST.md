...
1. 本机代码推送到 `origin/main` 后，由 origin 的 Webhook 通知公司服务器。
2. 公司服务器拉取 `origin/main`，生成可追溯发布信息并执行正式激活脚本。
3. 正式激活脚本强制重启服务，立即完成本机与公网验收。
4. 首次验收成功后，由公司服务器自身安排 30 分钟后的单次公网复验。
5. Webhook 是否成功不能只看文件是否同步；必须以公网 `/healthz` 返回的新提交号、构建号、源码指纹和全部工作进程一致为准。
