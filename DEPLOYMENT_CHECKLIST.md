...
正式激活脚本会为每次成功发布重新安排一个单次任务：30 分钟后再执行一次公网验收，验证提交号、构建号、源码指纹和 8 个 Gunicorn 工作进程。任务执行一次后即结束，不存在每小时或每天的循环巡检。执行结果必须写入 `CATALOG_POST_RELEASE_STATE`；公网 `/healthz` 中 `post_release_verification_current=true` 才能证明本次发布的延迟复验已经实际通过，而不只是定时配置存在。
...
```text
本机 -> origin/main -> 公司服务器 -> 公网
```

1. 本机代码推送到 `origin/main` 后，由既有 Webhook 自动通知公司服务器，日常发布不需要服务器管理员操作。
2. 公司服务器自动拉取 `origin/main`，刷新可追溯发布信息并执行正式激活脚本。
3. 正式激活脚本强制重启服务，立即完成本机与公网验收。
4. 首次验收成功后，由公司服务器自身安排 30 分钟后的单次公网复验。
5. Webhook 是否成功不能只看文件是否同步；必须以公网 `/healthz` 返回的新提交号、构建号、源码指纹和全部工作进程一致为准。若发布延迟或失败，应先排查自动发布链路，不得把日常发布转为依赖管理员手工操作。

`github` 只保存最新代码备份，不配置连接公司服务器的 Webhook、部署任务或公网验收任务。推送到 `github` 不会触发正式发布，也不能作为服务器已启用新版本的依据。是否备份到 `github` 必须遵循当次明确的推送范围。
