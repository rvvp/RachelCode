...
class PlanningApplication:
    ...
    def __call__(self, environ, start_response):
        ...
        try:
            ...
            if path == "/rules" and method == "GET":
                return self.html_response(start_response, self.render_rules(user))
    ...
    def handle_category_rule(self, environ, start_response, user):
        ...
        form = self.parse_form(environ)
        db.save_category_rule(self.db_path, form.get("season_year", ""), form.get("category", ""), float(form.get("multiplier") or 0), form.get("note", ""))
        return self.redirect(start_response, "/rules?notice=" + self.q("品类倍率已保存。"))
    ...
    def render_login(self, error: str = "") -> str:
        content = f"""
        <main class='login'><div class='login-mark'>PC / 商品企划中心</div><h1>上新定价工作台</h1><p class='muted'>连接藏宝阁的待定价新款，统一完成倍率、系数与价格带判断。</p>
        {self.alert(error, 'error') if error else ''}
        <form method='post' action='/login'><label>账号<input name='username' autofocus required></label><label>密码<input name='password' type='password' required></label><button class='primary' type='submit'>进入工作台</button></form>
        </main>"""
    ...
    def render_dashboard(self, user: dict) -> str:
        ...
        content = f"""
        <section class='hero'><div><div class='eyebrow'>MERCHANDISE PLANNING</div><h1>商品企划中心</h1><p>从藏宝阁读取已提交的新款资料，在同一个工作台完成上新价格决策并回传。</p></div><div class='hero-note'><span>当前操作人</span><strong>{html.escape(user.get('display_name',''))}</strong><small>{'管理员' if user.get('role') == 'admin' else '商品部企划员'}</small></div></section>
    ...
    def render_rules(self, user: dict) -> str:
        categories = db.list_category_rules(self.db_path)
        suppliers = db.list_supplier_coefficients(self.db_path)
        category_rows = ''.join(f"<tr><td>{html.escape(item['season_year'] or '默认')}</td><td>{html.escape(item['category'])}</td><td>{item['multiplier']:g}</td><td>{html.escape(item['note'])}</td></tr>" for item in categories)
        ...
        <section class='split'><div class='panel'><div class='panel-head'><h2>品类固定倍率</h2><span class='hint'>季节 + 品类优先</span></div><form class='rule-form' method='post' action='/rules/category'><input name='season_year' placeholder='年份季节，留空为默认'><input name='category' placeholder='品类，例如 毛衣' required><input name='multiplier' type='number' min='0.01' step='0.01' placeholder='4.00' required><input name='note' placeholder='备注'><button class='primary' {disabled}>保存倍率</button></form><div class='table-wrap'><table><thead><tr><th>适用季节</th><th>品类</th><th>倍率</th><th>备注</th></tr></thead><tbody>{category_rows or '<tr><td colspan="4" class="empty">尚未配置，系统默认使用 1.00。</td></tr>'}</tbody></table></div></div><div class='panel'><div class='panel-head'><h2>供应商浮动系数</h2><span class='hint'>未配置时为 1.00</span></div><form class='rule-form' method='post' action='/rules/supplier'><input name='season_year' placeholder='年份季节，留空为默认'><input name='supplier' placeholder='供应商名称' required><input name='coefficient' type='number' min='0.01' step='0.01' placeholder='1.00' required><input name='note' placeholder='备注'><button class='primary' {disabled}>保存系数</button></form><div class='table-wrap'><table><thead><tr><th>适用季节</th><th>供应商</th><th>系数</th><th>备注</th></tr></thead><tbody>{supplier_rows or '<tr><td colspan="4" class="empty">尚未配置，系统默认使用 1.00。</td></tr>'}</tbody></table></div></div></section>
        """
    ...
    def shell(self, title: str, content: str, user: dict, current: str) -> str:
        nav = ''.join(f"<a class='{'active' if current == key else ''}' href='{href}'>{label}</a>" for key, href, label in [("dashboard", "/dashboard", "企划总览"), ("workbench", "/workbench", "上新定价"), ("rules", "/rules", "定价规则"), ("stats", "/stats", "价格带统计"), ("settings", "/settings", "连接设置")])
