    ...
    def handle_pricing_batch(self, environ, start_response, user):
        ...
        if action == "approve":
            for record in records:
                ...
                if db.validated_launch_price(submitted_price) != db.validated_launch_price(record["launch_price"]):
                    raise ValueError(f"{record['style_code'] or record['product_name']} 的复核上新价已修改，请先点击“修改保存”。")
            for record in records:
        ...
        (() => {{
          ...
          document.querySelectorAll('.review-approval-form').forEach((form) => {{
            ...
            const syncApprovalState = () => {{
              ...
              approveButton.disabled = priceChanged;
              if (batchCheckbox) {{
                batchCheckbox.disabled = priceChanged;
                if (priceChanged) batchCheckbox.checked = false;
              }}
              updateBatchControls();
            }};
