...
class PlanningApplication:
    ...
    def handle_submit_review(self, environ, start_response, user, record_id: int):
        ...
        return self.redirect(start_response, "/workbench?notice=" + self.q(f"{record['style_code'] or record['product_name']} 已提交企划管理员审核。"))
    ...
    def handle_review_save(self, environ, start_response, user, record_id: int):
        ...
        return self.redirect(start_response, "/workbench?notice=" + self.q(f"{record['style_code'] or record['product_name']} 的审核上新价已保存。"))
    ...
    def handle_approve(self, environ, start_response, user, record_id: int):
        ...
        return self.redirect(start_response, "/workbench?notice=" + self.q(f"{record['style_code'] or record['product_name']} 已审核通过，可回传藏宝阁。"))
    ...
    def render_workbench(self, user: dict, query: dict) -> str:
        ...
        for item, record, workflow_status in filtered_products:
            ...
            source_fields = f"""
              <div class='source-field'><span>年份季节</span><strong>{html.escape(item.get('season_year') or '未提供')}</strong></div>
              <div class='source-field'><span>款号</span><strong>{html.escape(item.get('style_code') or '未提供')}</strong></div>
              <div class='source-field'><span>款色</span><strong>{html.escape(item.get('style_color') or item.get('color_name') or '未提供')}</strong></div>
              <div class='source-field source-field-image'><span>图片</span>{image}</div>
              <div class='source-field'><span>商品名称</span><strong>{html.escape(item.get('product_name') or '未提供')}</strong></div>
              <div class='source-field'><span>供应商</span><strong>{html.escape(item.get('supplier') or '未提供')}</strong></div>
              <div class='source-field'><span>含税成本</span><strong class='cost-value'>{html.escape(f"{float(cost):g}" if cost is not None else '未提供')}</strong></div>
              <div class='source-field'><span>来源状态</span><strong><span class='status status-source'>{html.escape(source_status_label)}</span></strong></div>
            """
            pricing_panel = ""
            if not record:
                category_value = html.escape(item.get("category", ""), quote=True)
                pricing_panel = f"""
                <div class='pricing-panel pricing-panel-empty'>
                  <div class='pricing-panel-head'><div><span class='eyebrow'>PRICING MATCH</span><h3>品类与规则匹配</h3></div><span class='status status-waiting'>待计算</span></div>
                  <form class='pricing-calc-form' method='post' action='/pricing/suggest'>
                    <input type='hidden' name='product_id' value='{item['id']}'>
                    <label>实际品类<input name='category' value='{category_value}' placeholder='例如 毛衣' required></label>
                    <button class='primary' type='submit' {' ' if can_price else 'disabled'}>计算建议价</button>
                  </form>
                ...
                if record_status in {"suggested", "conflict"} and user.get("role") != "admin":
                    controls = f"""
                    <form class='price-review-form' method='post' action='/pricing/{record['id']}/submit-review'>
                      <label>上新价<input name='launch_price' type='number' min='0.01' step='1' value='{price_value}' required></label>
                      <button class='primary' type='submit'>确认并提交审核</button>
                    </form>
                    <small>可先修改上新价，再提交给企划管理员审核。</small>"""
                elif record_status == "review_pending" and user.get("role") == "admin":
                    controls = f"""
                    <div class='review-controls'><span class='review-note'>商品部已提交，请审核上新价</span>
                      <form class='price-review-form' method='post' action='/pricing/{record['id']}/review-save'><label>审核上新价<input name='launch_price' type='number' min='0.01' step='1' value='{price_value}' required></label><button type='submit'>保存审核价</button></form>
                      <form class='price-review-form' method='post' action='/pricing/{record['id']}/approve'><label>最终上新价<input name='launch_price' type='number' min='0.01' step='1' value='{price_value}' required></label><button class='primary' type='submit'>审核通过</button></form>
                    </div>"""
                elif record_status == "confirmed" and user.get("role") == "admin":
                    controls = f"<form method='post' action='/pricing/{record['id']}/publish'><button class='primary' type='submit'>回传藏宝阁</button></form>"
                ...
                <div class='pricing-panel'>
                  <div class='pricing-panel-head'><div><span class='eyebrow'>PRICING MATCH</span><h3>品类与计算</h3><p>{html.escape(record['category'])} · {html.escape(rule_summary)}</p></div><span class='status status-{status_class}'>{html.escape(status_label)}</span></div>
                  <div class='price-result'><span>当前上新价</span><strong>{price_value}</strong><small>定价记录：{html.escape(record['publication_id'])}</small></div>
                  <div class='pricing-controls'>{controls}</div>{record_error}
                </div>"""
            cards.append(f"""
            <article class='product-card'>
              <div class='product-card-head'><div><span class='eyebrow'>SOURCE PRODUCT</span><h2>{html.escape(item.get('style_code') or item.get('product_name') or f"#{item['id']}")}</h2></div><span class='source-version'>来源 V{int(item.get('source_version_no') or 1)}</span></div>
              <div class='source-fields'>{source_fields}</div>
              {pricing_panel}
            </article>""")
        content = f"""
        <section class='page-heading'><div><div class='eyebrow'>NEW ARRIVAL PRICING</div><h1>上新定价工作台</h1><p>一款一张卡片完成资料查看、规则匹配、商品部确认、管理员审核和回传。</p></div><form method='post' action='/sync'><button class='primary' type='submit'>同步藏宝阁</button></form></section>
        {self.alert(notice, 'success') if notice else ''}{self.alert(error, 'error') if error else ''}
        <section class='filter-bar'><form method='get' action='/workbench'><label>年份季节<select name='season_year'><option value=''>全部季节</option>{''.join(f"<option value='{html.escape(value, quote=True)}' {'selected' if value == season else ''}>{html.escape(value)}</option>" for value in seasons)}</select></label><label>定价状态<select name='status'><option value=''>全部状态</option><option value='waiting' {'selected' if status == 'waiting' else ''}>待计算</option><option value='suggested' {'selected' if status == 'suggested' else ''}>待商品部确认</option><option value='review_pending' {'selected' if status == 'review_pending' else ''}>待管理员审核</option><option value='confirmed' {'selected' if status == 'confirmed' else ''}>审核通过，待回传</option><option value='published' {'selected' if status == 'published' else ''}>已回传</option><option value='conflict' {'selected' if status == 'conflict' else ''}>版本冲突</option></select></label><button type='submit'>筛选</button></form></section>
        ...
        <section class='product-card-list'>{''.join(cards) if cards else '<div class="panel empty">暂无符合条件的款色。请同步藏宝阁或调整筛选条件。</div>'}</section>
        """
    ...
    def record_status_label(self, status: str) -> str:
        return {"suggested": "待商品部确认", "review_pending": "待管理员审核", "confirmed": "审核通过，待回传", "published": "已回传", "conflict": "版本冲突", "failed": "回传失败"}.get(status, status)
