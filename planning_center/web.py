...
class PlanningApplication:
    ...
    def render_system_rules(self, user: dict) -> str:
        ...
        <section class='panel system-rule-section' id='review-flow'>
          ...
          <ul class='system-rule-list'>
            ...
            <li><strong>企划管理员：</strong>复核上新价格和上新渠道，可先保存修改再复核通过；不负责与藏宝阁之间的传输动作。渠道标准选项为天猫、天猫官、天猫奥、唯品、同款；回传后由藏宝阁按运营账号渠道属性分配可见范围。</li>
    ...
    def render_workbench(self, user: dict, query: dict) -> str:
        ...
        batch_toolbar = f"""
        <form id='pricing-batch-form' class='pricing-batch-form pricing-batch-inline' method='post' action='/pricing/batch'>
    ...
    def render_rules(self, user: dict, query: dict | None = None) -> str:
        query = query or {}
        # Keep this table and every category selector on the same active
        # option set, with the rules page as the source of truth.
        categories = db.list_category_options(self.db_path, enabled_only=True)
    ...
    def css(self) -> str:
        ...
        @media(max-width:620px){.pricing-board>.panel-head{flex-direction:column}.pricing-board-tools{align-items:flex-start;flex-direction:column;gap:6px;width:100%}.pricing-pagination-top{width:100%;justify-content:center;overflow-x:auto;flex-wrap:nowrap}.pricing-table[data-resizable-columns]{min-width:0}.product-image-dialog{width:calc(100vw - 20px);height:calc(100vh - 20px)}.product-image-dialog-head{padding:11px 10px 10px 14px}.product-image-dialog-canvas{padding:10px}}
