...
def seed_demo_users(connection: sqlite3.Connection) -> None:
    ...
    demo_users = [
        ...
        ("admin_reviewer", "资料审核管理员", "ADMIN", hash_password(DEMO_PASSWORD), 1, 0, now),
    ]
...
def seed_sample_products(connection: sqlite3.Connection) -> None:
    ...
    for department, sample in zip(("A", "B"), sample_rows, strict=True):
        product_id = create_product(connection, sample, users[department]["id"], department)
        if department == "A":
...
def update_product(connection: sqlite3.Connection, product_id: int, raw_values: dict, actor_user_id: int | None = None) -> None:
    ...
    if actor_user_id:
        connection.execute(
            """
            UPDATE products
            SET status = 'draft', last_reviewed_by = NULL, last_reviewed_at = NULL, updated_at = ?
            WHERE id = ?
            """,
...
def product_logs_csv_bytes(logs: list[dict]) -> bytes:
        ...
        rows.append(
            {
                ...
                "资料归属部门": item.get("owner_department", ""),
    ...
    writer = csv.DictWriter(
        buffer,
        fieldnames=["资料ID", "商品名称", "款号", "资料归属部门", "时间", "操作人", "部门", "动作", "说明", "字段差异"],
    )
    writer.writeheader()
