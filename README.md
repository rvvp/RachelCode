...
每次正式发布成功后，激活脚本会安排一个单次任务，30 分钟后再验证公网提交号、构建号、源码指纹和全部 Gunicorn 工作进程；执行完即结束，不做小时级循环巡检。GitHub Actions 也会在 `github/main` 更新 30 分钟后独立比对公网提交，用于发现 Gitea 部署 Webhook 完全漏发。
