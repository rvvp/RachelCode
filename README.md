...
正式服务器同步代码后，必须执行 `./scripts/activate_production_release.sh <公网根地址>`。该命令会强制重启 Gunicorn，并通过构建号、源码指纹、进程启动时间和连续公网抽查确认新代码已真正加载。

正式发布链路固定为：`本机 -> origin/main -> 公司服务器 -> 公网`。`origin/main` 的 Webhook 负责通知公司服务器拉取并激活新版本；服务器必须立即完成本机和公网验收，成功后再安排一个单次任务，30 分钟后复验公网提交号、构建号、源码指纹和全部 Gunicorn 工作进程。复验执行一次后即结束，不做小时级循环巡检。
