...
- 仅从藏宝阁读取状态为“A/B协作中”、生命周期正常且已上传图片的新款资料
...
- 品类、上新价格和上新渠道由商品企划中心唯一维护，藏宝阁只读展示并仅接受内部回传接口写入；图片和资料完成标记仍由藏宝阁商品部维护
...
- 协作开始后，`A` 可以继续补充其余主体资料，`B` 可以同步维护图片和资料完成标记，双方更新不会互相覆盖
- `A` 提交时可以附带交接说明
- `B` 只能维护 `图片` 和 `资料完成`；企划字段统一由商品企划中心维护并回传
